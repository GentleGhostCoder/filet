...
def mypy(session: Session) -> None:
    ...
    args = session.posargs or ["filet", "tests", "docs/conf.py"]
...
def tests(session: Session) -> None:
    """Run the test suite."""
    # Change the working directory to somewhere neutral, like the temp directory
    session.run("pip", "install", "ninja")
    session.run("pip", "install", "poetry")
    session.run("pip", "install", "poetry-plugin-export")
    session.run("poetry", "install")
...
def typeguard(session: Session) -> None:
    ...
    session.run("pip", "install", "pydantic")
    session.run("pip", "install", "ninja")
    session.run("pip", "install", "poetry")
    session.run("pip", "install", "poetry-plugin-export")
    session.run("poetry", "install")
...
def xdoctest(session: Session) -> None:
    ...
    os.environ["PIP_NO_BINARY"] = "pytype"
    session.run("poetry", "install")
